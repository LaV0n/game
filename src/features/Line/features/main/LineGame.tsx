...
export const LineGame = () => {
   ...
   const drop = () => {
      ...
      if (kindOfCoin !== 26) {
         ...
         conditionUp =
            Math.min(...coins) === +dragItem.current &&
            (panels.length === 0 ? true : dragItem.current > panels[panels.length - 1])
         conditionDown =
            Math.max(...coins) === +dragItem.current &&
            (panels.length === 0 ? true : dragItem.current < panels[panels.length - 1])
      } else {
         itemIndex = coins.findIndex(f => f === dragItem.current)
         conditionUp =
            Math.min(...coins.map(c => c.charCodeAt(0) - 97)) ===
               dragItem.current.charCodeAt(0) - 97 &&
            (panels.length === 0 ? true : dragItem.current > panels[panels.length - 1])
         conditionDown =
            Math.max(...coins.map(c => c.charCodeAt(0) - 97)) ===
               dragItem.current.charCodeAt(0) - 97 &&
            (panels.length === 0 ? true : dragItem.current < panels[panels.length - 1])
      }
      ...
      if (
         itemIndex !== -1 &&
         dragDiv.current === 'panel' &&
         (direction === 'up' ? conditionUp : conditionDown)
      ) {
         const dragItemContent = coins[itemIndex]
         ...
         <div className={styles.container}>
            ...
            <div className={styles.panel} id="panel" onDragEnter={dragEnter}>
               ...
               <div className={styles.panelWithHole}>
                  {[...Array(numberOfItem)].map((p, index) => (
                     <div className={styles.hole} key={index}></div>
                  ))}
               </div>
            </div>
         </div>
